		...
		public override void AI()
		{
			...
			if (!TailSpawned)
			{
				...
				for (int num36 = 0; num36 < 10; num36++)
				{
					...
					if (num36 >= 0 && num36 < 9)
					{
						lol = NPC.NewNPC((int)npc.position.X + (npc.width / 2), (int)npc.position.Y + (npc.width / 2), ModContent.NPCType<UndeadWyrmBody>(), npc.whoAmI);
					}
					else
					{
						lol = NPC.NewNPC((int)npc.position.X + (npc.width / 2), (int)npc.position.Y + (npc.width / 2), ModContent.NPCType<UndeadWyrmTail>(), npc.whoAmI);
					}
