...
namespace Tremor.NPCs.Bosses.Motherboard
{
	...
	public class Stage2 : Stage
	{
		...
		private void CheckClampers(Motherboard boss)
		{
			...
			{
				NPC npc = Main.npc[x];
				return npc.active && npc.type == ModContent.NPCType<Clamper>();
			}).ToList();
	...
	public class Motherboard:TremorModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			bossBag = ModContent.ItemType<MotherboardBag>();
		...
		public override void NPCLoot()
		{
			...
			else
			{
				...
				{
					this.NewItem((short)ModContent.ItemType<SoulofMind>(), Main.rand.Next(20, 40));
				}
				if (Main.rand.NextBool())
				{
				...
				{
					this.NewItem((short)ModContent.ItemType<MotherboardMask>());
				}
			}

			if (Main.rand.Next(10) == 0)
			{
				this.NewItem((short)ModContent.ItemType<MotherboardTrophy>());
			}
			if (Main.rand.NextBool(3))
			{
				this.NewItem((short)ModContent.ItemType<BenderLegs>());
			}
			if (Main.rand.Next(10) == 0)
			{
				this.NewItem((short)ModContent.ItemType<FlaskCore>());
			}

			if (NPC.downedMoonlord && Main.rand.NextBool())
			{
				this.NewItem((short)ModContent.ItemType<CarbonSteel>(), Main.rand.Next(6, 12));
			}
		}
