...
namespace Tremor.NPCs
{
	public class Dranix:TremorModNPC
	{
		...
		public override void HitEffect(NPC.HitInfo hit)
		{
			...
			{
				for (int k = 0; k < 20; k++)
					Dust.NewDust(npc.position, npc.width, npc.height, 74, 2.5f * hit.HitDirection, -2.5f);

				Gore.NewGore(null, npc.position, npc.velocity, Mod.GetGoreSlot("DranixGore1"), 1f);
				Gore.NewGore(null, npc.position, npc.velocity, Mod.GetGoreSlot("DranixGore2"), 1f);
				Gore.NewGore(null, npc.position, npc.velocity, Mod.GetGoreSlot("DranixGore2"), 1f);
				Gore.NewGore(null, npc.position, npc.velocity, Mod.GetGoreSlot("DranixGore3"), 1f);
				Gore.NewGore(null, npc.position, npc.velocity, Mod.GetGoreSlot("DranixGore3"), 1f);
