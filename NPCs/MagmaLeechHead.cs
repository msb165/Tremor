		...
		public override void AI()
		{
			...
			if (!TailSpawned && Main.netMode != 1)
			{
				...
				for (int i = 0; i < 14; i++)
				{
					int newNPCIndex = 0;
					if (i < 13)
						newNPCIndex = NPC.NewNPC((int)npc.position.X + (npc.width / 2), (int)npc.position.Y + (npc.width / 2), ModContent.NPCType<MagmaLeechBody>(), npc.whoAmI);
					else
						newNPCIndex = NPC.NewNPC((int)npc.position.X + (npc.width / 2), (int)npc.position.Y + (npc.width / 2), ModContent.NPCType<MagmaLeechTail>(), npc.whoAmI);
