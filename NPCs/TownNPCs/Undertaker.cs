...
namespace Tremor.NPCs.TownNPCs
{
	...
	public class Undertaker:TremorModNPC
	{
		...
		{
			shop.AddUniqueItem(ref nextSlot, ModContent.ItemType<Skullheart>());
			shop.AddUniqueItem(ref nextSlot, ModContent.ItemType<SpearofJustice>());
			shop.AddUniqueItem(ref nextSlot, ModContent.ItemType<TheGhostClaymore>());
			if (!Main.dayTime)
				shop.AddUniqueItem(ref nextSlot, ModContent.ItemType<LivingTombstone>());
		}
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			if (npc.life <= 0)
			{
				...
				for (int i = 0; i < 3; i++)
					Gore.NewGore(null, npc.position, npc.velocity, Mod.GetGoreSlot($"Gores/TheUndertakerGore{i+1}"), 1f);
			}
		}
	}
}