...
{
	public class UndeadMusketeer:TremorModNPC
	{
		...
		public override void NPCLoot()
		{
			if (Main.rand.Next(22) == 0)
				this.NewItem(ModContent.ItemType<MusketeerHat>());
			if (Main.rand.Next(25) == 0)
				this.NewItem(ModContent.ItemType<FencingRapier>());
			// Blunderbuss does not exist in the Tremor namespace.
			//if (Main.rand.Next(28) == 0)
			//	this.NewItem(ModContent.ItemType<BlunderBuss>());
		}
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			if (npc.life <= 0)
			{
				...
				Gore.NewGore(null, npc.position, npc.velocity, mod.GetGoreSlot("Gores/ZombieGore2"), 1f);
				Gore.NewGore(null, npc.position, npc.velocity, mod.GetGoreSlot("Gores/ZombieMusketeerGore"), 1f);
			}
		}
